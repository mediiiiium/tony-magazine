...
第1号 2026-08-20 は保管を始める前の号で生原稿が無かったため、刷り上がった誌面
`docs/issues/2026-08-20.html` から起こし直して保管庫に入れてある（組み直して誌面が
変わらないことは確認済み）。LLM の出力そのものではないので、扉の検索語と
トピックスの `data-date` だけは欠けている。原稿の先頭にその旨を書いてある。
